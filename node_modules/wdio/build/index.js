...
function run() {
    ...
    const program = new commander_1.Command('Create WebdriverIO')
        .version(pkg.version)
        .arguments('[project]')
    ...
    if (typeof projectName === 'undefined' && !fs_1.default.existsSync('package.json')) {
        ...
        console.log('To create WebdriverIO in a new project pass in a directory name:\n' +
            ...
            '\n' +
            'For example:\n' +
            `  ${chalk_1.default.cyan(program.name())} ${chalk_1.default.green('webdriverio-tests')}\n` +
            '\n' +
...
async function createWebdriverIO(opts) {
    ...
    useYarn = opts.useYarn || await utils_1.shouldUseYarn();
    let root = path_1.default.join(process.cwd(), projectName || '');
    if (!await utils_1.exists(root)) {
    ...
    if (!useYarn && !utils_1.checkThatNpmCanReadCwd()) {
        process.exit(1);
    }
    console.log(`\nCreating WebdriverIO project in ${chalk_1.default.bold(root)}\n`);
    const deps = ['@wdio/cli'];
    if (!await utils_1.exists(pkgJsonPath)) {
    ...
    await utils_1.runProgram('npx', ['wdio', 'config', ...(useYarn ? ['--yarn'] : []), ...(opts.yes ? ['--yes'] : [])]);
    console.log(`🤖 Successfully setup project at ${root} 🎉`);
    if (root != currentDir) {
        console.log(chalk_1.default.yellow('⚠'), 'Change directory to', chalk_1.default.yellow(root), 'to run these commands');
    }
}
function install(dependencies, root, opts) {
    ...
    return utils_1.runProgram(command, args);
}
